...
- **强引用**：`new`
- **软引用**：对于软引用关联的对象，在系统将要发生内存溢出异常之前，将会把对象列回收范围之中进行第二次回收。软引用的作用是告诉垃圾回收器，我新建的这个对象其实不那么重要（至少没有强引用简历的对象重要）。当JVM中的内存不足的时候，垃圾回收器可以释放这些只被软引用所指向的对象。
```java
...
```
- **虚引用**：为一个对象设置虚引用关联的目的为能在对象被收集器回收时收到一个系统通知。虚引用必须和引用队列 （ReferenceQueue） 联合使用。当垃圾回收器准备回收一个对象时，如果发现它还有虚引用，就会在回收对象的内存之前，把这个虚引用加入到与之关联的引用队列中，所以通过检查该队列里面的内容就知道一个对象是不是已经准备要被回收了。
...
把Java堆分为新生代和老年代，根哥哥年代的特点采用最适合的收集算法，新生代使用复制算法，老年代使用标记清楚或者标记整理

## HotSpot的算法实现

### 枚举根节点

可达性分析堆执行时间的敏感体现在GC停顿上，必须在一个能保证一致性的快照中进行。**一致性**：整个分析期间整个执行系统看起来像被冻结在某个时间点，不可以出现分析过程中堆像引用关系还在不断变化的情况->`Stop The World`

OopMap：在类加载完成的时候，把对象内的偏移量类型的数据计算出来，在特定的位置记录栈和寄存器中的那些位置是索引

### 安全点

程序只有到达安全点才能暂停，安全点不能选定太少，导致让GC等待时间过长，也不能过于频繁，一般会在方法调用、循环跳转、异常跳转等功能指令才会产生安全点

**如何在GC发生时让所有线程都到最近的安全点停下**

- 抢先式中断：把所有线程都中断，如果发现线程中断的地方不在安全点上，恢复线程
- 主动式中断：不直接对线程操作，设置一个标志，哥哥线程执行时主动轮询这个标志，发现终端标志为真时自己中断挂起

### 安全区域

对于处于Sleep和Blocked状态的线程无法响应JVM发容中断请求。安全区域是指在一段代码片段中，引用关系不会发生变化，在这个区域内的任意地方GC都是安全的
...
1. 并行与并发：使用多个CPU来缩短`Stop the World`停顿时间
2. 分代收集
3. 空间整合：G1整体基于`标记-整理`，局部基于`复制`，意味着运行过程中不会产生碎片，收集后能和提供规整的可用内存，分配大对象是不需要提前触发GC
4. 可预测的停顿

**不同Region中的对象引用**
虚拟机使用`Remembered Set`来比秒全队扫描
